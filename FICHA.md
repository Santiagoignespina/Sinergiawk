...
- Paleta (tokens en `components/vitrina/vitrina.css`): fondo hueso `#ece9e2`, placas `#ffffff`, tinta `#15161a`. Desde el logo nuevo (2026-09-21) el acento sale del logo: `--acento` `#0a6cf5` (un poco más profundo que el azul del logo, `#008bfc`, para que botones y textos pasen el contraste mínimo), `--degrade` (azul → celeste, en la tarjeta "50 proyectos"), `--degrade-raya` (las rayas de los títulos), `--celeste` `#06d1fc` (el del "IA") y `--error` `#d23a20` (los errores del formulario siguen en rojo).
...
- **Branding**: desde 2026-09-21 la página usa el **logo nuevo de Santiago**: el símbolo S (dos trazos con degradé azul → celeste) + SINERG + IA en celeste. El original es para fondo oscuro (SINERG en blanco); en la Vitrina, que es clara, SINERG va en el color del texto. Lo que NO se ve en la página sigue diciendo "Santiago Ignespina": título de la pestaña, metadata, JSON-LD e imagen para compartir (`app/opengraph-image.tsx`, todavía con la paleta oscura vieja). No cambiar sin que Santiago lo pida.
