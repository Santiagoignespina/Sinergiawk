...
- **Favicon** (`app/icon.png`, pestaña) y **`app/apple-icon.png`** (iPhone): desde 2026-09-21 son el bloque rojo con "ia" del logo actual (antes, el "si." oscuro de Santiago Ignespina). El logo entero no se lee a 16 px, por eso va solo el bloque. Se generaron sacando el "ia" tal como lo dibuja el sitio (misma letra y peso) sobre el acento `#e8442b`: el de la pestaña con las puntas redondeadas como el bloque del logo, el de iPhone a sangre porque el sistema redondea solo.
